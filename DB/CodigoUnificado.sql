...
--- PROCESO PARA REGISTRAR UN SOLICITUD
...
CREATE OR REPLACE PACKAGE PAQUETE_SOLICITUD AS
    ...
    FUNCTION MOSTRAR_SOLICITUDES RETURN SYS_REFCURSOR;
...
CREATE OR REPLACE PACKAGE BODY PAQUETE_SOLICITUD AS
    -- Procedure para agregar una solicitud
    PROCEDURE or replace RegistrarFormulario(
        p_id_user_solicitud IN NUMBER,
        p_fullname IN VARCHAR2,
        p_email IN VARCHAR2,
        p_phone IN VARCHAR2,
    ...
    -- Function para mostrar las solicitudes (usando un cursor)
    FUNCTION MOSTRAR_SOLICITUDES RETURN SYS_REFCURSOR AS
        c SYS_REFCURSOR;
    BEGIN
        OPEN c FOR SELECT * FROM SOLICITUD;
        RETURN c;
    END MOSTRAR_SOLICITUDES;
END PAQUETE_SOLICITUD;
/

--PRUEBA****
BEGIN
    PAQUETE_SOLICITUD.AGREGAR_SOLICITUD(
...
BEGIN
    PAQUETE_SOLICITUD.EDITAR_SOLICITUD(
        p_id_solicitud => 1,  -- ID de la solicitud que deseas editar
        p_id_user_solicitud => 5682,
...
BEGIN
    PAQUETE_SOLICITUD.ELIMINAR_SOLICITUD(
        p_id_solicitud => 1  -- ID de la solicitud que deseas eliminar
    );
END;
/
...
DECLARE
    ...
    v_email VARCHAR2(50);
BEGIN
    v_cursor := PAQUETE_SOLICITUD.MOSTRAR_SOLICITUDES;
    LOOP
        FETCH v_cursor INTO v_id_solicitud, v_fullname, v_email;
